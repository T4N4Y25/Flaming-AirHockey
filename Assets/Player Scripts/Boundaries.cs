...
public class Boundaries : MonoBehaviour //Code sourced from Retro Coder (Youtube): Source referenced in reflection
{
    ...
    void Start()
    {
        playerBoundary = new Boundary(transform.GetChild(0).position.y,transform.GetChild(1).position.y, transform.GetChild(2).position.x, transform.GetChild(3).position.x);
    ...
    void Update()
    {
        ...
        //if (OutsideXL)
       // {
         //   Vector2 stop = new Vector2((clampedX + 0.00001f), currentpos.y); Causes errors
       ...
       // }
        ...
        if (OutsideYT)
        {
            Vector2 stopt = new Vector2(currentpos.x, (clampedY - 0.000001f));
